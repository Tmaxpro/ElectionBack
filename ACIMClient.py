...
if __name__ == "__main__":
    ...
    client = ACIMSMSClient(
        username="votre_username",
        token="votre_token"
    )
    
    # Exemple 1: Envoi d'un SMS unique
    print("=== Envoi d'un SMS unique ===")
    result = client.send_one_sms(
        dest="2250554760285",
    ...
    if result['success']:
        print(f"Référence: {result.get('ref', 'N/A')}")
    print()
    
    # Exemple 2: Envoi de SMS en masse (bulk)
    print("=== Envoi de SMS en masse ===")
    messages = [
        {
            "Dest": "2250554760285",
            "Sms": "Message 1 pour le destinataire 1",
            "Sender": "ACIM INFO",
            "Flash": ""
        },
        {
            "Dest": "2250554760286",
            "Sms": "Message 2 pour le destinataire 2",
            "Sender": "ACIM INFO",
            "Flash": ""
        }
    ]
    
    result = client.send_bulk_sms(messages)
    print(f"Succès: {result['success']}")
    print(f"Statut: {result['status']}")
    if result['success']:
        print(f"Référence: {result.get('ref', 'N/A')}")
        print(f"Nombre de messages: {result.get('messages_count', 0)}")
    print()
    
    # Exemple 3: Récupération de l'accusé de réception
    print("=== Récupération de l'accusé de réception ===")
    if result['success'] and 'ref' in result:
        delivery = client.get_delivery_report(
            ref=result['ref'],
            dest="2250554760285"
        )
        print(f"Succès: {delivery['success']}")
        if delivery['success']:
            print(f"Délivré: {delivery.get('delivered', False)}")
            print(f"Statut SMC: {delivery.get('smc_description', 'N/A')}")
            print(f"Statut livraison: {delivery.get('delivery_description', 'N/A')}")
    print()
    
    # Affichage du résumé
    print("=== Résumé des envois ===")
    summary = client.get_sent_summary()
    print(f"Messages envoyés avec succès: {summary['total_sent']}")
    print(f"Messages échoués: {summary['total_failed']}")